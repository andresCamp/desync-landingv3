...
import GridItem from './GridItem3';
...
const LogoGrid: React.FC<LogoGridProps> = ({ logos, setSelectedLogo, selectedLogo }) => {
  return (
    <div className="md:w-2/3 lg:w-1/2 grid grid-cols-3 gap-4">
      {logos.map((logo) => (
        <GridItem key={logo.id} logo={logo} setSelectedLogo={setSelectedLogo} selectedLogo={selectedLogo} />
      ))}
    </div>
  );
};
